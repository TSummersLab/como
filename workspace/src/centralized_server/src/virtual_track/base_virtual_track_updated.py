...
def main():
    ...
    vehicle_sub = VehicleSub("COMO4")
    # todo: Update to use inherent namespace (from .bashrc or ROS launch files)
    ...
    # Required variables for velocity estimation are handled directly within the VehicleSub class
    ...
    while not rospy.is_shutdown():
        ...
        robot_pos, stamp = vehicle_sub.motive_sub.get_pose()
        timestamp_track.append(stamp)
        theta = vehicle_sub.motive_sub.get_yaw()

        # Velocity Estimation: Update position queue for velocity estimation module
        est_vel = vehicle_sub.est_vel_sub.get_velocity()
        vehicle_sub.est_vel_pub.estimate_velocity(robot_pos, stamp, est_vel)
        ...
        desired_speed = 1.8
        cur_speed = vehicle_sub.est_vel_sub.get_velocity()
