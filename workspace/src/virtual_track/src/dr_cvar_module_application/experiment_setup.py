...
from backend.dynamic_vehicles import CircularKinematicBicycle
...
def como_exp_setup(exp_type, metric, filter_slack, track):
    ...
    if exp_type == 'ego_overtaking':
        ...
        sim_steps = 8 # int(np.ceil(total_time / dt))

        # Collect sensor and external data for the specified ego vehicle
        ego_veh_id = "COMO4" #TODO: Modify to be selected via rosparam
        ego_veh_sub = VehicleSub(ego_veh_id, pose=[2.0, -1.0, 0], orientation=[0, 0, -np.pi/2], velocity=1.0)
        ...
        ego_init_state = np.hstack([ego_init_pos, -np.pi/2, 1.0, 0])
        ego_veh = CircularKinematicBicycle(ego_init_state, dt, t0, width/2, width/2, max_steer, ego_radius)
        ...
        for id in obst_veh_ids:
            ...
            obst_veh = CircularKinematicBicycle(obst_init_state, dt, t0, width/2, width/2, max_steer, obst_radius)
    ...
    # MPC Filter
    Q = np.diag([10, 10, 0, 0, 0])  # np.eye(ego_veh.n) * mpc_Q
    QT = np.diag([10, 10, 0, 0, 0]) # np.eye(ego_veh.n) * mpc_QT
    R = np.zeros(ego_veh.m) # np.eye(ego_veh.m) * mpc_R
    if filter_slack:
