...
from double_integrator_dynamics_model.experiment_setup import como_exp_setup
import pickle
...
def reach_avoid(seed, exp_type, metric, filter_slack, samp_dist='norm', realize_dist='lap',
    ...
    # Publisher for ego trajectory
    ego_traj_pub = Publisher('ego_traj', ego_traj, queue_size=1)
    ...
    ego_obst_dist_to_col = [[np.linalg.norm(ego_veh.get_pos - obst_veh.get_pos) - (ego_veh.radius + obst_veh.radius) for obst_veh in obst_vehs]]
    ...
    for sim_step in range(sim_steps):
        ...
        # TODO: Update ego_goal_state after each time state
        ...
        if not solved:
            raise NotImplementedError('Reference Trajectory error: Error generating reference trajectory')
        ...
        obst_pos_all.append(obst_pos_now)
        obst_polys_all.append(obst_polys_now)
        ego_obst_dist_to_col.append([np.linalg.norm(ego_veh.get_pos - obst_veh.get_pos) - (ego_veh.radius + obst_veh.radius) for obst_veh in obst_vehs])
    ...
    # Publish generated ego_trajectory
    ego_traj_data = ego_traj(mpc_filter.x_mpc[0], mpc_filter.x_mpc[1])
    ego_traj_pub.publish(ego_traj_data)
    ...
    # plot_sim_cvx_pb_data(mpc_filter_time_info, mpc_filter_status, safe_hs_time_info, OBST_COLORS, show=show_cvxpy_data, save=plot_cvxpy_data, save_file_name=fig_name + '_cvx_data.png')

    # plot_ego_obst_dist_to_collision(ego_obst_dist_to_col, OBST_COLORS, show=show_col_dist, save=plot_col_dist, save_file_name=fig_name + '_col_dist.png')
...
def main():
    reach_avoid(seed=2023, exp_type='ego_overtaking', metric='drcvar', filter_slack=False,
                ...
                xlim=(-3, 3), ylim=(-5, 4), figsize=None)
